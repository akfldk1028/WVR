...
namespace LaunchGame
{
    ...
    public class PrewarmGame : MonoBehaviour
    {
        ...
        private IEnumerator<float> Loader_Everything()
        {
            ...
            {
                // [DK 임시] QualityData가 없으면 스킵
                if (m_qualityData?.CurrentPreset?.ShaderVariants == null)
                {
                    Debug.LogWarning("[DK 임시] QualityData가 없어서 Shader 프리워밍을 스킵합니다. 나중에 수정 필요!");
                    yield return 1f;
                    yield break;
                }
            ...
            {
                // [DK 임시] QualityData가 없으면 스킵
                if (m_qualityData?.CurrentPreset?.ShaderVariantsSO == null)
                {
                    Debug.LogWarning("[DK 임시] QualityData가 없어서 렌더링 프리워밍을 스킵합니다. 나중에 수정 필요!");
                    yield return 1f;
                    yield break;
                }
            ...
            // TODO: Add collision material pairs 
            // TODO: Add motion vector support
        ...
        public class PrewarmRenderPass : ScriptableRenderPass
        {
            ...
            {
                // [DK 임시] QualityData가 없으면 스킵 - 나중에 수정 필요!
                if (PrewarmGame.m_qualityData?.CurrentPreset?.ShaderVariantsSO == null)
                {
                    Debug.LogWarning("[DK 임시] QualityData가 없어서 RenderVariants를 스킵합니다. 나중에 수정 필요!");
                    return;
                }
